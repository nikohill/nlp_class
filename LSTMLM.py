...
class TextLSTM(nn.Module):
    ...
    def forward(self, X):
        X = self.C(X)
        X = X.transpose(0, 1) 
        h_t_1 = torch.zeros(batch_size, n_hidden)  # [num_layers(=1) * num_directions(=1), batch_size, n_hidden]
        c_t_1 = torch.zeros(batch_size, n_hidden)     # [num_layers(=1) * num_directions(=1), batch_size, n_hidden]
